...
async function saveMessage(chatId, renterId, message) {
  ...
  const serializedId = message.id && message.id._serialized ? message.id._serialized : `${chatId}:${message.timestamp}:${direction}`;
  ...
  `, [
    serializedId, chatId, renterId, direction, null,
    message.body || "", message.type || "unknown", ts,
    JSON.stringify({
      from: message.from,
      to: message.to,
...
async function updateRenterDates(renterId) {
  await pool.query(`
    ...
    FROM (
      ...
      FROM whatsapp_messages WHERE renter_id=$1 GROUP BY renter_id
    ) x
...
app.get("/", (req,res) => {
  ...
  h1{font-family:Georgia,serif;font-weight:400;font-size:44px}.tag{color:#ba9251;text-transform:uppercase;letter-spacing:.12em;font-size:12px}
  .box{background:#fff;padding:24px;border:1px solid #d8d1c4;margin:18px 0}a{color:#15342c}
  </style></head><body>
  <div class="tag">張園 / Zheungyuan</div><h1>WhatsApp rental reader</h1>
  <div class="box">Status: <b>${waReady ? "Connected" : "Waiting for WhatsApp"}</b></div>
  <div class="box"><a href="/qr">Open QR pairing</a><br><br><a href="/status">Status JSON</a></div>
  <div class="box">When connected: POST <code>/sync/chats</code> to import recent chats.</div>
  </body></html>`);
});

app.get("/qr", (req,res) => {
  if (waReady) return res.type("html").send("<h2>WhatsApp is connected.</h2><p><a href='/'>Back</a></p>");
  if (!qrDataUrl) return res.type("html").send("<h2>Waiting for QR…</h2><meta http-equiv='refresh' content='3'>");
  res.type("html").send(`<h2>Scan with WhatsApp</h2><img src="${qrDataUrl}" width="320"><p>This QR refreshes automatically if required.</p><meta http-equiv='refresh' content='20'>`);
});

app.get("/status", (req,res) => res.json({ ok:true, waReady, ...syncState }));

app.post("/sync/chats", async (req,res) => {
  if (!waReady) return res.status(409).json({ ok:false, error:"WhatsApp not connected" });
  if (syncState.running) return res.status(409).json({ ok:false, error:"Sync already running" });

  const limitChats = Math.max(1, Math.min(Number(req.body.limitChats || 50), 250));
  const messagesPerChat = Math.max(10, Math.min(Number(req.body.messagesPerChat || 100), 500));

  syncState.running = true;
  let chatsSeen = 0, messagesSaved = 0;
  try {
    ...
    for (const chat of selected) {
      ...
      for (const message of messages) {
        messagesSaved += await saveMessage(chat.id._serialized, renterId, message);
      }
      await updateRenterDates(renterId);
    }

    syncState = { running:false, lastSync:new Date().toISOString(), chatsSeen, messagesSaved };
...
ensureSchema()
  ...
  .then(() => app.listen(port, "0.0.0.0", () => console.log(`Reader listening on ${port}`)))
  .catch(err => {
